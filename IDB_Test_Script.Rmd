...
### V. Bivariate Regression Analysis

To better understand the relationship between demographcis and subjective riches, we will begin by running a basic bivariate
regression of subjective riches over income. This will help us determine two things:

1) Is income significantly related to subjective riches? In other words, does subjective riches meaningfully change with income?
2) To what extent does income explain the variation we see in subjective riches?



```{r ols, echo=FALSE, warning=FALSE, results="asis", fig.show="hold", out.width="50%"}
...
summ(model1)
summ(model2)
#stargazer(model1, model2, title = "Model 1", type = "html", align=TRUE)
